...
IGA systems are responsible for evaluating the policies, making sure the data are compliant, addressing policy violations.
...
However, IGA platform should be aware of an existence of such applications, and relation of data store entitlements (such as LDAP groups) to the applications.
...
Most parts of the policy are processed in full-automatic mode, however, some parts require interaction with users and administrators.
